...
배경이 비어 버림. 그래서 **경계를 자료로 들고 감.** 쓰임이 다른 두 벌을 둠.

| 파일 | 내용 | 쓰임 |
|---|---|---|
| `서울시 행정경계(자치구-한강).geojson` | 자치구 `Polygon` 25개 + 한강 `LineString` | **배경** — 다섯 레이어가 공유하는 밑그림 |
| `서울시 상권영역(폴리곤).geojson` | 상권 `Polygon` 1,650개 (골목 1,090 · 발달 249 · 전통시장 305 · 관광특구 6) | **8-4 단계구분도의 도형** — 위험점수로 칠하는 면 |

상권 영역은 서울 열린데이터광장이 배포하는 Shapefile(`EPSG:5181`)을 위경도로 옮기고
**정점을 3m 공차로 줄여** GeoJSON으로 저장한 것임. 정점이 22만 개에서 4.5만 개로 줄지만
상권 하나가 수백 m 규모라 모양은 눈에 띄게 달라지지 않고, 파일이 1.5MB로 내려가 그대로 열림.

**폴리곤이므로 «점이 어느 구 안에 있는가»를 판정할 수 있음**(중첩분석). 아래에서 상권 좌표를
자치구 폴리곤에 넣어 자료의 자치구 열과 대조함 — 좌표·경계·자치구 세 자료가 서로 맞는지 보는 확인임.
...
_fc = json.loads((RAW / "서울시 행정경계(자치구-한강).geojson").read_text(encoding="utf-8"))
...
# 상권 영역 — 8-4 단계구분도에서 칠할 면. 상권 하나가 여러 조각인 경우가 있어 조각 단위로 폄
SK_GEO = json.loads((RAW / "서울시 상권영역(폴리곤).geojson").read_text(encoding="utf-8"))
SK_RINGS = {}
for _f in SK_GEO["features"]:
    _g = _f["geometry"]
    _rs = _g["coordinates"] if _g["type"] == "Polygon" else [p[0] for p in _g["coordinates"]]
    SK_RINGS[_f["properties"]["상권_코드"]] = [np.array(r) for r in _rs]
...
print(f"배경 — 자치구 폴리곤 {len(GU_POLY)}개 · 한강 유로 {len(HANGANG)}점")
print(f"상권 영역 — 폴리곤 {len(SK_RINGS):,}곳 · 조각 {sum(len(v) for v in SK_RINGS.values()):,}개 · "
      f"정점 {sum(len(r) for v in SK_RINGS.values() for r in v):,}개")

# 중첩분석(point-in-polygon) — 상권 좌표를 자치구 폴리곤에 넣어 판정하고 자료와 대조함
...
print(f"\n중첩분석 검증 — 상권 {len(base_pts):,}곳 · 경계 밖 {_out}곳 · 자료의 자치구와 일치 {_same:.1%}")
...
print("→ 어긋난 곳은 모두 «서로 맞닿은 두 구» 사이임. 자치구 경계선이 표시용으로 단순화된")
print("  폴리곤이라 경계에 붙은 상권이 옆 구로 넘어가는 것이며, 멀리 튄 사례는 없음.")
print("  배경으로는 그대로 쓰고, 구 소속은 자료의 «자치구» 열을 씀.")
```

    배경 — 자치구 폴리곤 25개 · 한강 유로 13점
    상권 영역 — 폴리곤 1,650곳 · 조각 1,786개 · 정점 45,226개
...
<table border="1" class="dataframe">
  ...
  <tbody>
    ...
    → 어긋난 곳은 모두 «서로 맞닿은 두 구» 사이임. 자치구 경계선이 표시용으로 단순화된
      폴리곤이라 경계에 붙은 상권이 옆 구로 넘어가는 것이며, 멀리 튄 사례는 없음.
      배경으로는 그대로 쓰고, 구 소속은 자료의 «자치구» 열을 씀.
...
## 8-4. 단계구분도 (주제도) — 상권 경계를 위험점수로 칠함

버블 분포도는 «점이 어디에 있는가»를 보여 주지만, 점의 크기는 규모이지 넓이가 아님.
상권은 실제로 **면**을 차지하므로, 그 면을 그대로 칠하면 「이 동네의 이만큼이 위험하다」가
넓이째로 읽힘. 칠하는 도형은 서울시가 배포하는 **상권 영역 폴리곤 1,650곳**임.

**같은 값을 두 단위로 칠해 나란히 놓음.**

| 왼쪽 | 오른쪽 |
|---|---|
| 상권 폴리곤 1,650곳 — 모델이 낸 위험점수 그대로 | 자치구 25곳 — 같은 값을 구 평균으로 뭉갠 것 |

**색 눈금을 두 장에서 똑같이 씀.** 그래야 「단위를 바꾸면 무엇이 사라지는가」가 보임 —
구 단위로 평균을 내면 값이 가운데로 몰려 지도가 밋밋해지고, 같은 구 안에서 갈리던 차이가
지워짐. 이것이 **대응 단위를 자치구가 아니라 상권으로 잡아야 하는 이유**를 그림 한 장으로
보여 주는 부분임.
...
# 상권 한 곳의 대표값 = 그 안에서 가장 위험한 업종의 점수 (8-3의 smap 과 같은 기준)
sk_risk = smap.set_index("상권_코드")["위험점수"]
...
NORM = mcolors.Normalize(0, 1)          # 두 장에서 같은 눈금 — 8-3 버블과도 같음
CMAP = "RdYlBu_r"

fig, axes = plt.subplots(1, 2, figsize=(15.6, 6.9))

# ── 왼쪽: 상권 폴리곤 ──
seoul_base(axes[0], labels=False, fill="#eceef1", edge="#d3d8de", river=True)
verts, vals = [], []
for cd, v in sk_risk.items():
    for ring in SK_RINGS.get(cd, []):
        verts.append(ring); vals.append(v)
pc = PolyCollection(verts, array=np.array(vals), cmap=CMAP, norm=NORM,
                    edgecolors="white", linewidths=.15, zorder=3)
axes[0].add_collection(pc)
plt.colorbar(pc, ax=axes[0], shrink=.78, label="예측 위험점수")
axes[0].set_title(f"상권 경계 단위 — 폴리곤 {len(sk_risk):,}곳", weight="bold", fontsize=12)

# ── 오른쪽: 같은 값을 자치구로 뭉갬 ──
gu_v = smap.groupby("자치구", observed=True)["위험점수"].mean()
sm = cm.ScalarMappable(norm=NORM, cmap=CMAP); sm.set_array([])
seoul_base(axes[1], labels=False, fill="#eceef1", edge="white", river=False)
for g_, P in GU_POLY.items():
    axes[1].add_patch(MplPolygon(P, closed=True, ec="white", lw=1.1, zorder=1,
                                 fc=sm.to_rgba(gu_v[g_]) if g_ in gu_v.index else "#e8eaed"))
    if g_ in gu_v.index:
        axes[1].text(*GU_CENTER[g_], f"{g_[:-1]}\n{gu_v[g_]:.2f}", fontsize=7.4, color="#111",
                     ha="center", va="center", zorder=3,
                     path_effects=[pe.withStroke(linewidth=2.2, foreground="white")])
axes[1].plot(HANGANG[:, 0], HANGANG[:, 1], c="white", lw=3, alpha=.8,
             solid_capstyle="round", zorder=2)
plt.colorbar(sm, ax=axes[1], shrink=.78, label="예측 위험점수 (구 평균)")
axes[1].set_title("자치구 단위 — 같은 값을 25곳으로 뭉갬", weight="bold", fontsize=12)

plt.tight_layout(); plt.savefig(FIG / "gis_choropleth.png", dpi=140); plt.show()

print(f"상권 단위  {sk_risk.min():.2f} ~ {sk_risk.max():.2f}  (폭 {sk_risk.max()-sk_risk.min():.2f})")
print(f"자치구 단위 {gu_v.min():.2f} ~ {gu_v.max():.2f}  (폭 {gu_v.max()-gu_v.min():.2f})")
print(f"→ 구 평균으로 뭉개면 값의 폭이 {(sk_risk.max()-sk_risk.min())/(gu_v.max()-gu_v.min()):.1f}배 줄어듦.")
print("  구 안의 분산이 구 사이의 분산보다 크다는 뜻이고, 그래서 대응 단위는 상권임.\n")

# 구 안에서 얼마나 갈리는지 — 분산분해
_g = smap.groupby("자치구", observed=True)["위험점수"]
ss_w = float(((smap["위험점수"] - _g.transform("mean")) ** 2).sum())
ss_b = float(((_g.transform("mean") - smap["위험점수"].mean()) ** 2).sum())
print(f"위험점수 분산분해 — 구 안 {ss_w/(ss_w+ss_b):.1%} · 구 사이 {ss_b/(ss_w+ss_b):.1%}")
...
print(f"구 단위 예측 순위 vs 실측 순위 — 스피어만 상관 {rho:.3f}")
print(f"  예측 상위 5 {' · '.join(top_p)}")
print(f"  실측 상위 5 {' · '.join(top_a)}")
print(f"  둘 다 {len(both)}개: {' · '.join(both) if both else '없음'}")
```


    
![png](img/%EC%83%81%EA%B6%8C_%ED%8F%90%EC%97%85%EC%9C%84%ED%97%98_%EC%A0%84%EC%B2%B4%EB%B6%84%EC%84%9D_124_0.png)
    


    상권 단위  0.19 ~ 0.87  (폭 0.67)
    자치구 단위 0.47 ~ 0.56  (폭 0.09)
    → 구 평균으로 뭉개면 값의 폭이 7.3배 줄어듦.
      구 안의 분산이 구 사이의 분산보다 크다는 뜻이고, 그래서 대응 단위는 상권임.
    
    위험점수 분산분해 — 구 안 96.8% · 구 사이 3.2%
...
<table border="1" class="dataframe">
  ...
  <tbody>
    ...
    구 단위 예측 순위 vs 실측 순위 — 스피어만 상관 0.442
      예측 상위 5 마포구 · 강남구 · 서초구 · 송파구 · 관악구
      실측 상위 5 마포구 · 금천구 · 강남구 · 송파구 · 양천구
      둘 다 3개: 강남구 · 마포구 · 송파구
